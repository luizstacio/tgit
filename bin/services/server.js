...
function styleRequest (req, res) {
  ...
  res.end(fs.readFileSync(pathTgit + '/bin/public/style.css'));
}
...
function pageSuccess (req, res) {
  ...
  res.end(jade.renderFile(pathTgit + '/bin/public/success.jade'));
...
function pageConfig (req, res) {
  trello.listBoards(function(data){
    res.writeHead(200, { 'Content-type': 'text/html' });
    res.end(jade.renderFile(pathTgit + '/bin/public/index.jade', data));
  });
}
